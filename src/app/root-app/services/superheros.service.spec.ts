...
const listSuperHero: SuperherosI[] = [
  {
    ...
    name: 'Spiderman',
    description:
      'Apodado el hombre araña, Peter Parker tras ser picado por una araña en un laboratorio adquiere superpoderes',
    photo:
      'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQc0I8xypw9ssz4LBRWZu-BzbSDhCuxzQKV8Q&usqp=CAU',
    powers:
  ...
  },
  {
    ...
    photo:
      'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTLw5ubh9p0hhOd-iDbSYiVFGf4oZ_bD2HCOg&usqp=CAU',
    powers:
      'Repulsores de energía y misiles de proyección. Regenerativo soporte vital.',
    battle_numbers: 69,
  },
];
...
describe('SuperherosService', () => {
  ...
  it('should get all the super heroes successfully', () => {
  ...
  it('should get a super heroes by id successfully', () => {
    service
      .getSuperHeroById(listSuperHero[0].id!!)
      .subscribe((response: SuperherosI) => {
  ...
  it('should create a superhero successfully', () => {
    service
      .createSuperHeros(listSuperHero[0])
      .subscribe((response: SuperherosI) => {
  ...
  it('should edit a superhero satisfactorily', () => {
    service
      .updateSuperHeros(listSuperHero[0])
      .subscribe((response: SuperherosI) => {
        expect(response.name).toEqual(listSuperHero[0].name);
      });

    const req = httpMock.expectOne(
      `${environment.url}/${listSuperHero[0].id}`,
      listSuperHero[0].id
    );
